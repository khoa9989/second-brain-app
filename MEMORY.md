...
- **Protocol Cập nhật Pháp lý (priority workflow)**: When anh Khoa says "TÔM, có [Tên văn bản], thực hiện Protocol Cập nhật Pháp lý", execute the full 6-step legal update workflow immediately without asking again: (1) find/verify/normalize and store in Vault/SecondBrain/Legal_Library/Local/HaiPhong as Markdown with metadata, (2) update Legal_Master_Index and archive superseded texts, (3) scan Projects for old citations and replace or flag them, (4) sync to Second Brain/Netlify, (5) assess audit/progress risk, (6) send one batch report.
- **Single Source of Truth cho Netlify Sync**: Luôn thao tác trực tiếp và đồng bộ từ thư mục `Vault/` nằm bên trong repo `second-brain-app` (`/home/openclawtom/.openclaw/workspace/second-brain-app/Vault/`). Tuyệt đối không tạo hay duy trì các bản sao rời rạc bên ngoài repo. Mọi file cập nhật theo quy trình pháp lý phải được lưu trực tiếp vào cấu trúc này để lệnh `git push` mang nội dung chuẩn lên Netlify ngay lập tức.
